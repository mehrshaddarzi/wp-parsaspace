...
class Wp_Parsaspace_Admin_Ajax {
    ...
    public function change_setting_domain_parsaspace() {
        ...
        if ( defined('DOING_AJAX') && DOING_AJAX ) {
            ...
            if($is_cron_change_update_link ===true) {
                ...
                //Get New Url
                $http = "http://";
                if( trim($_GET['is_ssl']) =="yes") $http = "https://";
                $base_folder_new = '';
                if ( trim($base_folder) !="" ) { $base_folder_new = '/'.$opt['base_folder']; }
    ...
    public function request_remote_url_to_parsaspace() {
        ...
        if ( defined('DOING_AJAX') && DOING_AJAX ) {
            ...
            //Check Not $_GET[]
            if( !isset($_GET['file_url']) ) exit;
            if( !isset($_GET['file_name']) ) exit;
    ...
    */
                ...
                if($list_file ===false) {
                    ...
                    if($count_item ==0) {
                        ...
                        foreach($list_file as $l) {
                            ...
                            //time
                            $date = explode("T", $l['LastModified'])[0];
                            $time = explode(".", explode("T", $l['LastModified'])[1])[0];
    ...
    public function wp_remove_path_parsaspace() {
        ...
        if ( defined('DOING_AJAX') && DOING_AJAX ) {
            ...
            //Check Not $_GET[]
            if( !isset($_GET['path_remove']) ) exit;
