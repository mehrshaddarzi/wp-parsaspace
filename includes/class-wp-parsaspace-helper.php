...
class Wp_Parsaspace_Helper {
    ...
    public function get_count_attachment($type = false,$get_size = false) {
        ...
        if($type) {
            //image
            if($type =="image") { $arg['post_mime_type'] = array('image'); }
            //audio
            if($type =="audio") { $arg['post_mime_type'] = array('audio'); }
            //video
            if($type =="video") { $arg['post_mime_type'] = array('video'); }
            //asnad
            if($type =="app") { $arg['post_mime_type'] = array('application', 'text'); }
            //me
            if($type =="me") { $arg['author'] = get_current_user_id(); }
