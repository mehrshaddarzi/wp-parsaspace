...
class Wp_Parsaspace_Admin {
    ...
    public function Check_is_Basefolder() {
        ...
        if ( trim($opt['base_folder']) !="" ) { return '/'.$opt['base_folder']; } else { return false; }
    }
    ...
    public function get_base_parsaspace_url() {
        ...
        //Check Basefolder
        $base_folder = '';
        if ( trim($opt['base_folder']) !="" ) { $base_folder = '/'.$opt['base_folder']; }
...
افزونه را غیر فعال کنید
    ...
    public function handle_bulk_actions_upload($redirect_to, $doaction, $post_ids ) {
        ...
        foreach ( $post_ids as $post_id ) {
            if ( get_post_meta($post_id, 'cdn_parsaspace', true) ==1 ) { } else {
                $file_id = $post_id;
                $time = time() + $time_out;
    ...
    public function check_attachment_in_cron($post_id) {
        ...
        foreach($list_key as $k) {
            $key = $k[0];
            if( $ids[$q][$key]['args'][0] ==$post_id) { return true; }
            $q++;
        }
    ...
    public function upload_size_limit($size) {
        $size = 1024 * 1024 * (get_option('wp_parsaspace_opt')['max_size_upload']);
        return  $size;
    }
